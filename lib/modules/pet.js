...
class Pet {
  ...
  print(message) {
    ...
    //상태 변수 가져오기
    const level = this.petStatus.get("level");
    const exp = this.petStatus.get("exp");
    const hunger = this.petStatus.get("hunger");
    ...
    statusPrinter.print(this._getStatusStr("Exp", `${exp}/20`));
